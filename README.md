...
2. **UI-тест** `test_ui_full_meal_plan.py` выполняет полный цикл создания, проверки и удаления плана питания (выбор типа питания → рецепт → дата → сохранение). В CI запускается в headless-режиме. При нестабильной работе сервера тест может требовать повторного запуска.
3. Отображение "ERROR" в календаре: на тестовом сервере Eduson в некоторых ячейках календаря Meal Plan отображается надпись "ERROR" вместо названия рецепта. Это связано с битыми ссылками на рецепты на стороне сервера и не является ошибкой автотестов.
